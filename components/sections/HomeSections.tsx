    ...
    <>
      <section className="dark-mesh pt-32 text-ink md:pt-40">
        <div className="mx-auto grid max-w-7xl items-center gap-12 px-4 pb-24 md:grid-cols-2 md:px-6">
          <Reveal><p className="mb-4 text-sm font-bold uppercase tracking-[0.18em] text-electric">Adaptive AI systems for teams with real work to move.</p><h1 className="font-display text-5xl font-bold tracking-tight md:text-7xl">Shape Smarter Operations With avorynai.</h1><p className="mt-6 text-lg leading-8 text-muted">avorynai builds practical AI platforms that coordinate work, surface decisions, support customers, and keep critical processes moving.</p><div className="mt-8 flex flex-wrap gap-3"><Button href="/contact">Start Your Project</Button><Button href="/solutions" variant="secondary">Explore Our Solutions</Button></div></Reveal>
      ...
      <SectionWrap dark><SectionHeading eyebrow="Process" title="How avorynai builds clarity into work" text="We map the operating reality first, then ship systems that are easy to adopt, audit, and improve." /><div className="grid gap-4 md:grid-cols-3">{processSteps.map((s, i) => <Reveal key={s.title} delay={i * 0.04}><div className="rounded-lg border border-electric/10 bg-white/80 p-6 text-ink shadow-sm"><span className="text-sm font-bold text-electric">0{i + 1}</span><h3 className="mt-3 font-display text-xl font-bold">{s.title}</h3><p className="mt-3 text-sm leading-7 text-muted">{s.description}</p></div></Reveal>)}</div></SectionWrap>
      ...
      <section className="bg-white px-4 py-20 text-center md:px-6"><Reveal><h2 className="font-display text-4xl font-bold md:text-5xl">Ready to Connect the Work?</h2><p className="mx-auto mt-4 max-w-2xl text-muted">Share the workflow, backlog, or decision process you want to improve. avorynai will help turn it into a working system.</p><div className="mt-8 flex justify-center gap-3"><Button href="/contact">Start a Project</Button><Button href="/contact" variant="ghost">Contact avorynai</Button></div></Reveal></section>
    </>
  );
}
